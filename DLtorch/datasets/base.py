...
class BaseCVDataset(BaseDataset):
    def __init__(
        ...
        self.train_transforms = transforms.Compose([getattr(transforms, _trans)(**tran_transforms[_trans]) for _trans in train_transforms.keys()])
        self.test_transforms = transforms.Compose([getattr(transforms, _trans)(**tran_transforms[_trans]) for _trans in test_transforms.keys()])
